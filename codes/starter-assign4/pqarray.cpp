...
void PQArray::reallocateMem(){
    _numAllocated *= 2;
    DataPoint* new_elements = new DataPoint[_numAllocated]();
...
void PQArray::enqueue(DataPoint elem) {
    ...
    // find a place to insert elem in a sorted Array
    int swapIndex = _numFilled - 1;


    while ( swapIndex >= 0 && elem.priority > _elements[swapIndex].priority) {
        swap(swapIndex, swapIndex + 1);
        swapIndex--;
    }

    _elements[swapIndex + 1] = elem;
    _numFilled++;
    return;
}
