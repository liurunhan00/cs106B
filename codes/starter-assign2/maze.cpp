...
void validatePath(Grid<bool>& maze, Stack<GridLocation> path) {
    ...
    if (!maze[path.peek().row][path.peek().col]) {
        error("Path can not reach at maze exit");
    }
    /* TODO: Fill in the remainder of this function. */
    Set<GridLocation> trackPath;
    GridLocation top;
    GridLocation prev;
    prev = path.pop();
    trackPath.add(prev);
    while (!path.isEmpty()) {
        top = path.pop();
        if (!generateValidMoves(maze, top).contains(prev)) {
            error("Path generate invalid moves");
        }
        if (trackPath.contains(top)) {
            error("Path contains same grid or the gird counldn't reach");
        }
        trackPath.add(top);
        prev = top;
    }
...
Stack<GridLocation> solveMaze(Grid<bool>& maze) {
    ...
    Stack<GridLocation> front;
    /* TODO: Fill in the remainder of this function. */
    Queue<Stack<GridLocation>> curPaths;
    GridLocation mazeExit = {maze.numRows()-1,  maze.numCols()-1};
    GridLocation mazeEntry = {0, 0};

    front.push(mazeEntry);
    curPaths.enqueue(front);

    while (!curPaths.isEmpty()) {
        front = curPaths.dequeue();
        if (front.peek() == mazeExit) {
            return front;
        }
        auto curNode = front.pop();
        GridLocation prevNode;
        bool prevIsEmpty;
        if (!front.isEmpty()) {
            prevNode = front.peek();
            prevIsEmpty = false;
        ...
        auto validateMoves = generateValidMoves(maze, curNode);
        front.push(curNode);

        for (auto valid: validateMoves) {
            if (prevIsEmpty || prevNode != valid) {
                front.push(valid);
                curPaths.enqueue(front);
                front.pop();
            }
...
STUDENT_TEST("generateValidMoves on location on the side of 3x3 grid with three walls") {
